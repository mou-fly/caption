...
class Dataset_IMU(Dataset):
    def __init__(self, root_path, label_dict, path, flag='train', size=None,
    ...
    #     path_list1s = [0, 84, 112]
    #     path_list2s = [84, 112, 141]
    #     # num_list = [84, 28, 29]
    #     path1 = path_list1s[self.set_type]
    #     path2 = path_list2s[self.set_type]
    #     # num_iter = num_list[self.set_type]
    #     all_data = []
    #     all_label = []
    #     # len_csv = 0
    #     for i in range(path1, path2):
    #         temp_path = r'C:\project\Time-LLM\dataset\reorganized_dataset\session{0}\all_acc_gyro.csv'.format(i)
    ...
    #     # 创建一个空的列表，用于存储所有行
    #     all_rows = []
    #     # 遍历所有的二维矩阵，将它们的行添加到 all_rows 列表中
    #     for matrix in all_data:
    #         all_rows.extend(matrix)  # 添加所有行
    #
    #     self.data_x = np.array(all_rows)
    #     self.data_y = np.array(all_rows)
    #
    #     all_rows = []
    #     # 遍历所有的二维矩阵，将它们的行添加到 all_rows 列表中
    #     for matrix in all_label:
    #         all_rows.extend(matrix)  # 添加所有行
    #     # print(len(all_rows))
    #     self.data_label = np.array(all_rows)

    '''用于间隔性地获取dataset，0-93为train_set，94-141为test_set'''
    def __read_data__(self):
        self.scaler = StandardScaler()
    ...
    def __getitem__(self, index):
        ...
        # seq_y_label = self.data_label[r_begin:r_end]
    ...
    def __len__(self):
        return (len(self.data_x) - self.seq_len - self.pred_len + 1) * self.enc_in // self.stride
