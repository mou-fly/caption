...
class DecoderBlock(nn.Module):
    def __init__(self, configs):
        ...
        self.transformer_block = EncoderBlock(configs)
        self.dropout = nn.Dropout(configs.dropout)

    def forward(self, x, y, src_mask=None, tgt_mask=None):
        text_encode = self.attn(x, x, x, tgt_mask)
        attention = self.dropout(self.norm(text_encode + x))
        out = self.transformer_block(attention, y, y)
        return out, text_encode
...
class Decoder(nn.Module):
    def __init__(self, configs):
        ...
        self.device = configs.device
        self.d = configs.transformer_d_model
        self.n_layers = configs.transformer_n_layers
        self.text_len = configs.caption_max_len
    ...
    def forward(self, x, enc_out, src_mask=None, tgt_mask=None):      
        ...
        # 1) 词嵌入
        word_emb = self.word_embedding(x)            # [batch_size, seq_len, d]

        # 2) 生成位置索引，并做一次 Embedding
        #    positions: [seq_len]
        positions = torch.arange(seq_len, device=self.device)
        #    pos_emb: [seq_len, d]
        pos_emb = self.position_embedding(positions)

        # 3) 加法时，pos_emb 会自动广播成 [batch_size, seq_len, d]
        x = word_emb + pos_emb                       # broadcasting！
        ...
        # 5) 后续解码层 & 投影
        for layer in self.layers:
            x, text_encode = layer(x, enc_out, src_mask, tgt_mask)
        out = self.proj(x)                           # [batch_size, seq_len, vocab_size]
        return out, text_encode
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.pe == 'b':
            ...
            self.pos_emb = PositionalEmbeddingLearnable(self.n_frames, self.raw_dim, self.device)
        elif self.pe == 'tl':
            self.pos_emb = PositionalEmbedding(self.n_frames, self.raw_dim, self.device)
            self.pos_lin = nn.Linear(self.raw_dim, self.raw_dim)
    ...
    def forward(self, src, tgt, src_mask=None, tgt_mask=None):
        ...
        if self.use_patch:
            ...
            if self.transformer_patch:
                assert patch_num == self.n_frames
                src = src.view(batch, n_device, patch_num, -1)
            else:
                src = src.view(batch, n_device, -1, dim)
        batch, n_device, length, dim = src.shape
        if self.device_index != 0:
            src = self.device_mask(src)
        if self.joints_emb and self.transformer_patch==False:
        ...
        src = src.permute(0, 2, 1, 3).contiguous().view(batch, length, -1)
        if self.pe == 't':
            pe = self.pos_emb(src)
            # print(pe.shape)
            # print("src:", src.shape)
        ...
        enc_out = self.encoder(src, src, src).permute(0, 2, 1).contiguous()
        enc_out = self.Linear2(enc_out).permute(0, 2, 1).contiguous()  # 16 30 512
        out, text_encode = self.decoder(tgt, enc_out, src_mask, tgt_mask)
        if self.itc_loss:
            t_f = text_encode[:,0,:].squeeze(1)
            i_f = enc_out[:,0,:].squeeze(1)
            loss = itc_loss(i_f, t_f)
            return self.proj(out), loss
